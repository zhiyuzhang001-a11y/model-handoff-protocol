...
The separate `.model-handoff/update.py` helper accesses the fixed public upstream
only when explicitly invoked. It requires no interactive credentials, validates
the requested Git ref, fetches into a temporary directory without a shell, prints
the resolved commit, and delegates to the dry-run-first installer. It never
changes an existing destination file; `differs` and `conflict` require manual
review.
